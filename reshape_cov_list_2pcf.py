...
if load_mat_files:
    ...
    mm.matshow(cov_mat_fmt_2ddav, log=True, title='original, 2d Dav')
...
thetas = pd.read_csv(f'{cov_folder}/covariance_list.dat', delim_whitespace=True, usecols=['theta1'], names=column_names, header=0)['theta1'].unique()
theta_indices = {theta: idx for idx, theta in enumerate(thetas)}
...
for df_chunk in pd.read_csv(f'{cov_folder}/covariance_list.dat', delim_whitespace=True, names=column_names, skiprows=1, chunksize=chunk_size):
    ...
    # Now, work with NumPy arrays directly for faster processing
    ...
    # Vectorize the assignment to the cov_g_10d array
    for i, row in tqdm(enumerate(df_chunk.itertuples(index=False))):
        zi, zj, zk, zl = z_indices[i]
        cov_g_10d_test[probe_idxs[i, 0], probe_idxs[i, 1], theta1_indices[i], theta2_indices[i], zi, zj, zk, zl] = row.covg
        # cov_g_10d[indices[i, 0], indices[i, 1], theta1_indices[i], theta2_indices[i], *z_indices[i]] = row.covg
...
cov_g_6d = cov_g_10d[0, 0, ...]
cov_g_4d = mm.cov_6D_to_4D(cov_g_6d, theta_bins, zpairs_auto, ind_auto)
