                       ...
                       int *result)
{
    ...
    for (ind_gal = 0; ind_gal < ngal; ind_gal++)
    {
        ...
        {
            if (result[start_ngalinpix + index] == 0)
            {
                npixs_with_gals += 1;
            }
    ...
    {
        if (result[start_ngalinpix + ind_pix] == 0)
        {
            result[start_matcher + ind_pix] = FLAG_NOGAL;
        }
        else
        {
                    ...
                    for (ind_pix1 = pix1_lower; ind_pix1 < pix1_upper; ind_pix1++)
                    {
                        for (ind_pix2 = pix2_lower; ind_pix2 < pix2_upper; ind_pix2++)
                        {
                            ...
                            for (ind_inpix = lower; ind_inpix < upper; ind_inpix++)
                            {
                                ...
                                dist = sqrt(dist2);
                                if (dist < reso_redges[elreso] || dist >= reso_redges[elreso + 1])
                                {
                                    continue;
                                }
                                // dist = sqrt(dist2);
