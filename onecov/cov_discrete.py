...
class Catalog:
    ...
    def __init__(self, pos1, pos2, weight=None, zbins=None, isinner=None):
        ...
        sitepackages = get_site_packages_dir()
        self.clib = ct.CDLL(sitepackages+"/discretecov.cpython-310-x86_64-linux-gnu.so")
...
class XipmMixedCovariance(BinnedNPCF):
    ...
    def process(self, cat, nthreads=16, dotomo=True):
        ...
        _leninner = np.max(cat.pos1[cat.isinner.astype(np.bool)])-np.min(cat.pos1[cat.isinner.astype(np.bool)])
